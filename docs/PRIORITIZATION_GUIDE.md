...
## The registry is generated, not hand-edited

`.mwp/FEATURE_PRIORITY_REGISTRY.md` is a computed, disposable view — never edit it directly. Each feature scores itself in its own `FEATURE_META.md` (created at scaffold time, one file this feature's folder alone owns), and `scripts/registry.sh` regenerates the registry from every feature's metadata on demand. This avoids multiple parallel feature sessions all editing the same shared file — the same coordination-cost problem this framework flags for shared schemas, just self-inflicted in its own bookkeeping if done by hand. See `docs/evolution/0003-computed-priority-registry.md`.

## Execution rules once scored

1. **Core Data Anchor first** (unconditional, unscored) — see `.mwp/FEATURE_PRIORITY_REGISTRY.md`, regenerated by `scripts/registry.sh`.
