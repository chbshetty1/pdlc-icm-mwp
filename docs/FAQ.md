...
Mostly yes now, for the checks that have been adopted so far. Shared-schema collisions (`0005`) are mechanically checked: `sync.sh` greps a feature's `04_architecture_design/outputs/` and `05_development_test/src/` against a declared `shared_path:` list in `.mwp/GLOBAL_CONTEXT.md` and refuses to sync on a match, auto-writing a pre-filled `BLOCKED_REASON.md`. Token guardrails (`0004`) remain just a proposed artifact-level check — not yet implemented. The single most foundational constraint — that an agent only reads the files explicitly declared in its stage's `CONTEXT.md` — now has a real, if deliberately imperfect, check: every stage's `outputs/` includes a self-reported `Context_Manifest.md` (every file the agent actually read), and a human cross-checks it against the declared `READ ONLY` scope at the same Obsidian review gate everything else goes through. Extras are a flag, not an automatic block. **Neither the manifest nor the shared-path check is airtight** — the manifest is self-reported, not OS-level traced, and the shared-path check only catches a literal name match, not every possible way a shared definition could be touched indirectly. Both limitations are stated plainly rather than hidden. Full file-access tracing and a more thorough collision detector are the noted next steps if either proves insufficient in practice — deliberately not built until that's a concrete need. See `docs/evolution/0018-scope-containment-verification.md` and `docs/evolution/0005-automate-shared-schema-collision-check.md`.
