...
Partially. Token guardrails (`0004`) and shared-schema collisions (`0005`) are still just proposed artifact-level checks (easy, since the evidence is a file you can inspect after the fact — no action taken yet). But the single most foundational constraint — that an agent only reads the files explicitly declared in its stage's `CONTEXT.md` — now has a real, if deliberately imperfect, check: every stage's `outputs/` includes a self-reported `Context_Manifest.md` (every file the agent actually read), and a human cross-checks it against the declared `READ ONLY` scope at the same Obsidian review gate everything else goes through. Extras are a flag, not an automatic block. **This is not mechanical enforcement** — the manifest is self-reported, not traced at the OS/file-access level, so an agent could in principle omit an out-of-scope read from it. That limitation is stated plainly in `CRITICAL_ESCALATION.md` rather than hidden. Full file-access tracing (e.g. `strace`-equivalent) is the noted next step if self-reporting proves unreliable in practice — deliberately not built until that's a concrete need. See `docs/evolution/0018-scope-containment-verification.md`.
