...
At least one, caught the hard way: figuring out the correct relative path for a new stage-01 reference to `LESSONS_LEARNED.md` required actually counting directory depth (a scaffolded stage lives 3 levels below product root, at `features/<name>/<stage>/`), and that count didn't match the `../../` (2 levels) every existing stage `CONTEXT.md` already uses for references back to product-root files like `GLOBAL_CONTEXT.md` and `CRITICAL_ESCALATION.md` — those look one level short. Logged as entry `0025` rather than silently fixed, since it affects 6 files and deserves confirmation, not just one person's arithmetic. The general lesson: these relative paths are prompt text an LLM agent reads and interprets, not something any script mechanically resolves — so a wrong path here doesn't fail loudly, it just costs an agent a bit of self-correcting exploration each time, silently, indefinitely, until someone happens to count levels by hand. Worth double-checking path depth explicitly (not just pattern-matching the surrounding text) any time a new cross-file reference is added to a stage `CONTEXT.md`. See `docs/evolution/0025-relative-path-depth-audit.md`.
